...
namespace DSIGE.Negocio
{
    public class NLectura
    {
        ...
        public DataTable N_ListandoReparto_Tomadas(int servicio, string tipoRecibo, string cicloFacturacion, int Estado, string fecha_ini, string fecha_fin, string suministro, string medidor, int operario, string fechaAsignacion)
        {
            try
            {
                return new DLectura().D_ListandoReparto_Tomadas(servicio, tipoRecibo, cicloFacturacion, Estado, fecha_ini, fecha_fin, suministro, medidor, operario, fechaAsignacion);
            }
            catch (Exception e)
            {
                throw e;
