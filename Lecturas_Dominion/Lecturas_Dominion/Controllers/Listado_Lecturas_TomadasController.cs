...
namespace DSIGE.Web.Controllers
{
    public class Listado_Lecturas_TomadasController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ListandoReparto_Tomadas(int servicio, string tipoRecibo, string cicloFacturacion , int Estado, string fecha_ini, string fecha_fin, string suministro, string medidor, int operario, string fechaAsignacion)
        {
            return new ContentResult
            {
                Content = MvcApplication._Serialize(
                        new NLectura().N_ListandoReparto_Tomadas(servicio,   tipoRecibo,   cicloFacturacion,   Estado,   fecha_ini,   fecha_fin,   suministro,   medidor,   operario, fechaAsignacion)
                    , true),
                ContentType = "application/json"
            };
        }
        ...
        [HttpPost]
        public string Descarga_repartoExcel(int servicio, string tipoRecibo, string cicloFacturacion, int Estado, string fecha_ini, string fecha_fin, string suministro, string medidor, int operario, string fechaAsignacion)
        {
            int _fila = 2;
            string _ruta;
            string nombreServicio = "";
            ...
            {
                 NLectura obj_negocio = new DSIGE.Negocio.NLectura();
                _lista = obj_negocio.N_ListandoReparto_Tomadas(servicio, tipoRecibo, cicloFacturacion, Estado, fecha_ini, fecha_fin, suministro, medidor, operario, fechaAsignacion);
                ...
                using (Excel.ExcelPackage oEx = new Excel.ExcelPackage(_file))
                {
                    ...
                    oWs.Cells[1, 1].Value = "Nro Recibo";
                    oWs.Cells[1, 2].Value = "Suministro";
                    oWs.Cells[1, 3].Value = "Cuenta Contrato";
                    oWs.Cells[1, 4].Value = "Porcion"; 

                    oWs.Cells[1, 5].Value = "Ciclo";
                    oWs.Cells[1, 6].Value = "Fecha Recojo";
                    oWs.Cells[1, 7].Value = "Fecha Entrega";
                    oWs.Cells[1, 8].Value = "Unidad Lectura";
                    oWs.Cells[1, 9].Value = "Operario";
                    oWs.Cells[1, 10].Value = "Estado";
                    oWs.Cells[1, 11].Value = "Latitud";
                    oWs.Cells[1, 12].Value = "Longitud";
                    oWs.Cells[1, 13].Value = "Hora entrega";

                    oWs.Cells[1, 14].Value = "Nombre de Cliente";
                    oWs.Cells[1, 15].Value = "Telefono Fijo";
                    oWs.Cells[1, 16].Value = "Telefono Celular";

                    oWs.Cells[1, 17].Value = "Direccion de Facturacion";
                    oWs.Cells[1, 18].Value = "Urbanizacion de Facturacion";
                    oWs.Cells[1, 19].Value = "Distrito de Facturacion";
                    oWs.Cells[1, 20].Value = "Poblacion de Facturacion";
                    oWs.Cells[1, 21].Value = "Tipo de Cliente";

                    foreach (DataRow oBj in _lista.Rows)
                    {
                        oWs.Cells[_fila, 1].Value = oBj["nroRecibo"].ToString();
                        oWs.Cells[_fila, 2].Value = oBj["suministro"].ToString();
                        oWs.Cells[_fila, 3].Value = oBj["cuentaContrato"].ToString();
                        oWs.Cells[_fila, 4].Value = oBj["porcion"].ToString();
 
                        oWs.Cells[_fila, 5].Value = oBj["ciclo"].ToString();
                        oWs.Cells[_fila, 6].Value = oBj["fecha_recojo"].ToString();
                        oWs.Cells[_fila, 7].Value = oBj["fecha_entrega"].ToString();

                        oWs.Cells[_fila, 8].Value = oBj["unidad_lectura"].ToString();
                        oWs.Cells[_fila, 9].Value = oBj["operario"].ToString();
                        oWs.Cells[_fila, 10].Value = oBj["estado"].ToString();

                        oWs.Cells[_fila, 11].Value = oBj["latitud"].ToString();
                        oWs.Cells[_fila, 12].Value = oBj["longitud"].ToString();
                        oWs.Cells[_fila, 13].Value = oBj["hora"].ToString();

                        oWs.Cells[_fila, 14].Value = oBj["nombreCliente"].ToString();
                        oWs.Cells[_fila, 15].Value = oBj["telefonoFijo"].ToString();
                        oWs.Cells[_fila, 16].Value = oBj["telefonoCelular"].ToString();

                        oWs.Cells[_fila, 17].Value = oBj["direccionFacturacion"].ToString();
                        oWs.Cells[_fila, 18].Value = oBj["urbanizacionFacturacion"].ToString();
                        oWs.Cells[_fila, 19].Value = oBj["distritoFacturacion"].ToString();

                        oWs.Cells[_fila, 20].Value = oBj["poblacionFacturacion"].ToString();
                        oWs.Cells[_fila, 21].Value = oBj["tipoCliente"].ToString();
