...
namespace DSIGE.Dato
{
    public class Cls_Dato_ResultadoLecturas
    {
       ...
       public List<ResultadoLecturas_E> Capa_Dato_Get_ResumenLecturas(string FechaAsignacion, int id_tiposervicio, int id_supervisor, int id_operario_supervisor, string ciclo)
        ...
        public List<ResultadoLecturas_E> Capa_Dato_Get_ResumenReparto(string FechaAsignacion, int id_tiposervicio, string ciclo)
        {
            try
            {
                cadenaCnx = System.Configuration.ConfigurationManager.ConnectionStrings["dataSige"].ConnectionString;
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("NEW_SP_S_RESULTADO_REPARTO_RESUMEN", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
       ...
       public List<ResultadoLecturas_E> Capa_Dato_Get_ultimoSector()
       {
            ...
            try
            {
                ...
                using (Excel.ExcelPackage oEx = new Excel.ExcelPackage(_file))
                {
                    ...
                    foreach (DataRow oBj in dt_detalles.Rows)
                    {
                        ...
                        oWs.Cells[_fila, 5].Style.Numberformat.Format = "#";
                        oWs.Cells[_fila, 5].Value = Math.Round(Convert.ToDecimal(oBj["total"]), 2);
                        ...
                        oWs.Cells[_fila, 6].Style.Numberformat.Format = "#";
                        oWs.Cells[_fila, 6].Value = Math.Round(Convert.ToDecimal(oBj["conFoto"]), 2);
                        ...
                        oWs.Cells[_fila, 7].Style.Numberformat.Format = "#";
                        oWs.Cells[_fila, 7].Value = Math.Round(Convert.ToDecimal(oBj["sinFoto"]), 2);
        ...
        public List<ResultadoLecturas_E> Capa_Dato_Get_ResumenReparto_Detallado(string FechaAsignacion, int id_tiposervicio, string ciclo)
        {
            try
            {
                cadenaCnx = System.Configuration.ConfigurationManager.ConnectionStrings["dataSige"].ConnectionString;
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("NEW_SP_S_RESULTADO_REPARTO_DETALLE", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            foreach (DataRow Fila in dt_detalle.Rows)
                            {
                                ...
                                obj_entidad.diaUno = Fila["diaUno"].ToString();
                                obj_entidad.diaDos = Fila["diaDos"].ToString();
                                obj_entidad.diaTres = Fila["diaTres"].ToString();
