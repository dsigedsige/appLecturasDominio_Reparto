...
namespace DSIGE.Dato
{
    public class Cls_Dato_VerificacionFotos
    {
        ...
        public string Capa_Dato_set_reactivarCuentaContrato(int servicio, string fecha, string contrato,int idUsuario)
        {
            string Resultado = "";
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_U_VERIFICACION_FOTOS_REACTIVAR", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@idServicio", SqlDbType.Int).Value = servicio;
                        cmd.Parameters.Add("@fecha", SqlDbType.VarChar).Value = fecha;
                        cmd.Parameters.Add("@cuentaContrato", SqlDbType.VarChar).Value = contrato;
