...
namespace DSIGE.Dato
{
    public class Cls_Dato_CambioEstado
    {
       ...
       public string Capa_Dato_GenerarAnular_Masivo(int servicio, string fecha_Asigna, int estado, int operario, int usuario, string archivosImportado, int usuarioImportacion)
        {
            string Resultado = "";
            try
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_U_CAMBIO_ESTADO_DAR_BAJA_NEW", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        cmd.Parameters.Add("@Usuario", SqlDbType.Int).Value = usuario;
                        cmd.Parameters.Add("@archivosImportado", SqlDbType.VarChar).Value = archivosImportado;
                        cmd.Parameters.Add("@usuarioImportacion", SqlDbType.Int).Value = usuarioImportacion;
       ...
       public List<Cambio_Estado_Masivo_E> Capa_Dato_Get_ListaServicio()
       {
        ...
        public DataTable Capa_Dato_Get_archivosCargados(int id_servicio, string fecha_asignacion)
        {
            DataTable dt_detalle = new DataTable();
            try
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_S_CAMBIO_ESTADO_ARCHIVOS_CARGADOS", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataTable Capa_Dato_Get_usuariosImportacion(int id_servicio, string fecha_asignacion)
        {
            DataTable dt_detalle = new DataTable();
            try
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_S_CAMBIO_ESTADO_USUARIOS_IMPORTACION", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@id_servicio", SqlDbType.Int).Value = id_servicio;
