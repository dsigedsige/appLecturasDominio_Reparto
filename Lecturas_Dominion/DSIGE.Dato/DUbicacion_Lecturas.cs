...
namespace DSIGE.Dato
{
    public class DUbicacion_Lecturas
    {
        ...
        public List<Ubicacion_Lectura> Capa_Dato_Get_ListaOperariosGrandesClientes()
        {
            try
            {
                List<Ubicacion_Lectura> Listlocal = new List<Ubicacion_Lectura>();
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_S_OPERARIO_GRANDES_CLIENTES", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        DataTable dt_detalle = new DataTable();
        ...
        public List<Ubicacion_Lectura> Capa_Dato_Get_ListaOperarios_servicio( int idServicio )
        {
            try
            {
                List<Ubicacion_Lectura> Listlocal = new List<Ubicacion_Lectura>();
                using (SqlConnection cn = new SqlConnection(cadenaCnx))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SP_S_OPERARIO_SERVICIO", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@idServicio", SqlDbType.VarChar).Value = idServicio;
        ...
        public List<UbicacionOperario> DSeguimiento_Lectura_III(string pFechaAsiga, int servicio, int operario, string suministro, string medidor, string fechaEntrega)
        {
            List<UbicacionOperario> lOb = new List<UbicacionOperario>();
            try
            {
                //using (IDataReader iDr = DatabaseFactory.CreateDatabase().ExecuteReader("PROC_SEGUIMIENTO_OPERARIO_DATO_SUMINISTRO", operario, pFechaAsiga, suministro, medidor, 1, servicio, fechaEntrega))
                using (IDataReader iDr = DatabaseFactory.CreateDatabase().ExecuteReader("PROC_SEGUIMIENTO_OPERARIO_DATO_SUMINISTRO_II", operario, pFechaAsiga, suministro, medidor, 1, servicio, fechaEntrega))
                {
                    if (iDr != null)
                    {
                        while (iDr.Read())
