...
namespace DSIGE.Dato
{
    public class DLectura
    {
        ...
        public DataTable D_ListandoReparto_Tomadas(int servicio, string tipoRecibo, string cicloFacturacion, int Estado, string fecha_ini, string fecha_fin, string suministro, string medidor, int operario, string fechaAsignacion)
        {
            DataTable dt_detalle = new DataTable();
            try
            {
                ...
                {
                    cn.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_S_REPORTE_REPARTOS_II", cn))
                    {
                        cmd.CommandTimeout = 0;
                        cmd.CommandType = CommandType.StoredProcedure;
